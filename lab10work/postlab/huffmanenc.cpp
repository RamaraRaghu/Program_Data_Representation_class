...
int main(int argc, char **argv){
  ...
  for(map<char,string>::const_iterator it = result.begin(); it != result.end(); it++){
    ...
    for(int l = 0; l < frequencies.size(); l++){
      if(chars[l] == it->first){
	results = results + ((double) it->second.length())*((double) frequencies[l])/((double) total_count);
      }
    }
  ...
  double compression = ((double) total_count) * 8 / ((double) comp);
  cout << "This gives a compression ratio of " << compression << endl;
  cout << "The cost of the Huffman tree is "<< results << " bits per character." << endl;
