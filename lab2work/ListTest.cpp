//	Modified: 9/13/2018: rr5de
//  CS2150 Lab 2 - Test Harness
//   by Rakshith Raghu
//
//My forward and backward functions are opposite of what I think they should be. Other its implemented.
...
void printList(List& source, bool direction){
  ListItr *printer = new ListItr;
  if(source.size() >= 1){
    if(direction == true){
      *printer = source.first();
      while(printer->isPastEnd() == false){
	...
	printer->moveBackward();
      ...
      *printer = source.last();
      while(printer->isPastBeginning() == false){
	cout << printer->retrieve() << " ";
	printer->moveForward();
      }
      cout << endl;
