 ...
 * lab portions done by Rakshith Raghu, 10/25/2018
 * 3 for loops, one for loop of size 22, and one for loop of hashtable size
 * O(rows*columns*8*22*size of hashtable)
 */
...
//#define MAXROWS 500
//#define MAXCOLS 500
//going to just directly store it
char grid[500][500];

// Forward declarations
int  floorSqrt(int x); //taken from online source
...
int main(int argc, char* argv[]) {
  //getting the two line parameters
    string dictionary_file(argv[1]);
    string grid_file(argv[2]);

    vector<string> results;
    
    //getting the count for the dictionary
    string line;
    ifstream input(dictionary_file.c_str());
    ...
    hashTable diction(num_lines);

    line = "";
    ifstream inputer(dictionary_file.c_str());

    while(getline(inputer, line)){
      diction.insert(line);
    }
	...
	for(int k = 0; k < cols; k++){
	  ...
	  case 1:
	    help = cols - k-1;
	    if((j+1)*(j+1) + help*help < 22*22){
	      max_word_size = floorSqrt((j+1)*(j+1) + help*help);
	    }
	    else{
	      max_word_size = 22;
	    }
	  ...
	  case 3:
	    help = cols - k - 1;
	    helpp = rows - j - 1;
	    if(help*help + helpp*helpp ){
	      max_word_size = floorSqrt(help*help + helpp*helpp);
	    }
	    else{
	      max_word_size = 22;
	    }
	  ...
	  case 5:
	    help = rows - j - 1;
	    if(help*help + (k+1)*(k+1) < 22*22){
	      max_word_size = floorSqrt(help*help + (k+1)*(k+1));
	    }
	    else{
	      max_word_size = 22;
	    }
	  ...
	  for(int m = 3; m <= max_word_size; m++){
	    if(diction.inDictionary(getWordInGrid(j,k,i,m,rows,cols))){
	      found = string(getWordInGrid(j,k,i,m,rows,cols));
		  words_found = words_found + 1;
		  switch (i) {
		     case 0:
		       results.push_back("N (" + tostring(j) + ", " + tostring(k) + "):   " + found);
		       break;
		     case 1:
		       results.push_back("NE (" + tostring(j) + "," + tostring(k) + "):  " + found);
		       break;
		     case 2:
		       results.push_back( "E (" + tostring( j) + "," + tostring( k) +  "):   " + found);
		       break;
		     case 3:
		       results.push_back("SE (" + tostring(j) + "," + tostring(k) + "):    " + found);
		       break;
		     case 4:
		       results.push_back( "S (" + tostring(j) + "," + tostring(k) + "):    " + found);
		       break;
		     case 5:
		       results.push_back("SW (" + tostring(j) + "," + tostring(k)  + "):   " + found);
		       break;
		     case 6:
		       results.push_back("W (" + tostring(j) + "," + tostring(k) + "):    " + found);
		       break;
		     case 7:
		       results.push_back("NW (" + tostring(j) + "," + tostring(k) + "):    " + found);
		       break;
		  }
					}}}}}
    t.stop();
